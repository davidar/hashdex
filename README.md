...
| `hdx recipe disarchive <recipe>` | project a gzip-chain recipe into disarchive's sexp format (gzip-member → tarball → directory-ref), with the `swh:1:dir` identifier minted from the tar itself — no API asked. Every tar header is round-trip-verified through the field model before emitting; unrepresentable tars (pax/GNU extensions, sparse) refuse loudly. Stock disarchive assembles the result: for GNU hello our projection is token-identical to disarchive's own published recipe, and `disarchive assemble` rebuilds the tarball byte-exactly from it |
