//! Project a gzip-chain recipe into disarchive's sexp format, so
//! stock disarchive (Guix's tool) can rebuild the tarball from our
//! metadata — the same referee move as the jigdo projection.
//!
...
fn blob_hashes(prefix_len: u64, feed: impl FnOnce(&mut dyn FnMut(&[u8]))) -> ([u8; 20], [u8; 32]) {
...
impl Tree {
    fn insert(&mut self, path: &[u8], node: TNode) -> Result<()> {
...
mod tests {
    ...
    fn git_tree_matches_git_itself() {
        ...
        assert_eq!(hex_lower(&s1), "45b983be36b73c0788dc9cbcb76cbb80fc7bb057");
        ...
        .unwrap();
        let (t1, _) = t.hash();
        assert_eq!(hex_lower(&t1), "b0e66a8a93b83161375f18dcdc9e9329af61e04f");
    }
