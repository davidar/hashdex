...
pub fn all() -> Vec<Backend> {
    vec![
        ...
        Backend {
            name: datasets::DEBS.name,
            supports: datasets::DEBS.supports,
            lookup: |_, k| Box::pin(dataset::lookup(&datasets::DEBS, k)),
        },
        Backend {
            name: datasets::MEDIA.name,
            supports: datasets::MEDIA.supports,
            lookup: |_, k| Box::pin(dataset::lookup(&datasets::MEDIA, k)),
        },
        Backend {
            name: datasets::RPM_FILES.name,
            supports: datasets::RPM_FILES.supports,
            lookup: |_, k| Box::pin(dataset::lookup(&datasets::RPM_FILES, k)),
        },
        Backend {
            name: swh::NAME,
