...
enum Command {
    ...
    /// Scan directories and files: hash every file, check membership
    /// filters, and attribute what hdx can actually name (local
    /// indexes plus range reads over published datasets; --no-resolve
    /// for the pure membership census). An explicitly named container
    /// file gets the full recursive descent — the hashoscope: every
    /// nested member through tarballs, debs, rpms, zips, squashfs,
    /// isos, nested any way, hashed and identified like a file on disk
    Scan {
